...
const char* Scanner::TW[] = { "", "Boolean", "break", "continue", "do", "else", "false", "for", "function",
    "getenv", "if", "in", "NaN", "Number", "null", "Object", "return", "String", "true", "typeof",
...
void Parser::E()
{
    ...
    else
    {
        ...
        if (c_type == LEX_EQ || c_type == LEX_LSS || c_type == LEX_GTR || c_type == LEX_LEQ
            || c_type == LEX_GEQ || c_type == LEX_NEQ || c_type == LEX_DMINUS
            || c_type == LEX_DPLUS)
        {
...
void Parser::T()
{
    F();
    while (
        c_type == LEX_TIMES || c_type == LEX_SLASH || c_type == LEX_DAMP || c_type == LEX_PERCENT)
    {
        type_of_lex op = c_type;
        st_lex.push(c_type);
        gl();
...
void Parser::GV()
{
	...
	else
	{
		...
		{
			poliz.push_back(Lex(LEX_STR_CONST, 0, c_str));
			gl();
		}
		else
		{
		...
		{
			gl();
			poliz.push_back(Lex(POLIZ_GETENV));
		}
	}
}
/////////////////////////////////////////////////////////////////////////////
    ...
    while (index < size)
    {
        ...
        switch (pc_el.get_type())
        {
			...
			case POLIZ_WRITE:
				from_st(args, one);
				if (one.get_type() == POLIZ_ADDRESS)
					if (TID[one.get_value()].get_str()!="")
						cout<<TID[one.get_value()].get_str()<<'\n';
					else
						cout<<TID[one.get_value()].get_value()<<'\n';
			...
			case POLIZ_GETENV:
				from_st(args, one);
				cout<<getenv(one.get_string().c_str())<<'\n';
				break;
